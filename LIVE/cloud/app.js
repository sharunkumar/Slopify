...
async function uploadFile(file) {
    try {
        ...
        showNotification(`Failed to upload ${file.name}: ${error.message}`, 'is-danger');
    }
}
...
async function downloadFile(name) {
    try {
        ...
        showNotification(`Failed to download ${name}: ${error.message}`, 'is-danger');
    }
}
...
async function refreshCurrentFolder() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" class="has-text-centered">Loading...</td></tr>';
...
function updateBreadcrumb(path) {
    ...
    ul.innerHTML = '<li><a href="#" onclick="navigateToFolder(\'/\')">Root</a></li>';
