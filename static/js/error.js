...
class Windows95 {
    ...
    async errorLoop(e) {
        ...
        } else if (!code) { this.switchError(e); }
    }
    dragError(e) {
    ...
    dragErrorStart(e) {
        ...
        if (target?.nodeName !== "BUTTON") {
            ...
            if (headerFound) {
                ...
                } else { this.cursorDragPos = { x: e.clientX, y: e.clientY }; }
            }
        }
    }
    dragErrorEnd() {
    ...
    switchError(e) {
        ...
        let { target } = e;
        do { target = target?.parentElement; } while (target && !target?.hasAttribute("data-window"))
        if (target) {
